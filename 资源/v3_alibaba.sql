...
(
    user_id          int(11) auto_increment comment '用户表主键',
    ...
    real_name   varchar(128) not null default '' comment '用户真实姓名',
...
create table if not exists alibaba.user_role
(
    user_role_id int(11) auto_increment comment '用户-角色中间表主键',
...
values (1, 'zhaosi', 'zhaosi', '18211111111', '亚洲舞王', '赵国强', now(), now()),
       (2, 'liuneng', 'liuneng', '18222222222', '象牙山小诸葛', '刘能', now(), now()),
       (3, 'guangkun', 'guangkun', '18233333333', '首富', '赵国强', now(), now()),
       (4, 'xuzhengli', 'xuzhengli', '18244444444', '村主任', '徐正利', now(), now());
